    ...
    : d(new Internal::SftpChannelPrivate(channelId, sendFacility, this))
{
    ...
    connect(d, &Internal::SftpChannelPrivate::fileDownloadProgress,
            this, &SftpChannel::fileDownloadProgress, Qt::QueuedConnection);
    connect(d, &Internal::SftpChannelPrivate::fileUploadProgress,
            this, &SftpChannel::fileUploadProgress, Qt::QueuedConnection);
}
...
Internal::AbstractSftpOperation::Ptr SftpChannel::lookupJob(SftpJobId id)
{
    Internal::SftpChannelPrivate::JobMap::Iterator itr = d->m_jobs.find(id);
    if (itr != d->m_jobs.end())
    {
        return itr.value();
    }
...
void SftpChannelPrivate::handleReadData()
{
    ...
    emit fileDownloadProgress(op->jobId, op->fileSize,op->localFile->pos());
...
void SftpChannelPrivate::sendWriteRequest(const JobMap::Iterator &it)
{
    ...
    if (job->localFile->error() != QFile::NoError) {
        ...
        emit fileUploadProgress(job->jobId, job->localFile->size(), job->localFile->pos());
