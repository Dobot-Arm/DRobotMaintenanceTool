...
void RoundProgressBar::setDiyFontAndSmallText(QFont font, QString smallText)
{
    m_font = font;
...
void RoundProgressBar::paintText(QPainter& painter)
{
    if(!(_controlFlags&defaultText))
        return;
    int yOffset = 0;
    if(!m_smallText.isEmpty()){
        painter.setPen(QColor(138,143,160));
        painter.setFont(QFont(m_font.family(),m_font.pointSize()-14,m_font.weight()));
        painter.drawText(_squareStart,_squareStart+15,_squareWidth,_squareWidth,Qt::AlignCenter,
                         m_smallText);
        yOffset = -15;
    }
    ...
    case valueAndMax:
        ...
        break;
    case percent:
        painter.drawText(_squareStart,_squareStart+yOffset,_squareWidth,_squareWidth,Qt::AlignCenter,
                         QString::number(_value/_max*100,'f',_precision)+"%");
        break;
    case check:
        painter.drawText(_squareStart,_squareStart+yOffset-2,_squareWidth,_squareWidth,Qt::AlignCenter,
                         "x");
        //✔✓√✔✓\n╳×
        break;
    default:
